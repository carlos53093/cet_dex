...
The contract that deployed the pool, which must adhere to the ICryptoV3Factory interface
...
```solidity
struct CryptoV3Pool.Slot0 slot0
```
...
```solidity
struct CryptoV3Pool.ProtocolFees protocolFees
```
...
```solidity
contract ICryptoV3LmPool lmPool
```
...
_Prevents calling a function from anyone except the address returned by ICryptoV3Factory#owner()_
...
_The caller of this method receives a callback in the form of ICryptoV3SwapCallback#CryptoV3SwapCallback_
...
_The caller of this method receives a callback in the form of ICryptoV3FlashCallback#CryptoV3FlashCallback
...
```solidity
function setLmPool(contract ICryptoV3LmPool _lmPool) external
```
