...
import '@cryptoswap2/v3-periphery/contracts/base/PeripheryImmutableState.sol';
import '@cryptoswap2/v3-periphery/contracts/base/BlockTimestamp.sol';
import '@cryptoswap2/v3-periphery/contracts/libraries/Path.sol';
import '@cryptoswap2/v3-periphery/contracts/libraries/PoolAddress.sol';
import '@cryptoswap2/v3-core/contracts/interfaces/ICryptoV3Pool.sol';
import '@cryptoswap2/v3-periphery/contracts/libraries/OracleLibrary.sol';
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getBlockStartingAndCurrentTick(ICryptoV3Pool pool)
        internal
        view
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (ICryptoV3Pool pool) {
        pool = ICryptoV3Pool(PoolAddress.computeAddress(deployer, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            ICryptoV3Pool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = ICryptoV3Pool(pool).slot0();
            }
