...
describe('NonfungiblePositionManager', () => {
  ...
  describe('#mint', () => {
    ...
    it('fails if cannot transfer', async () => {
      ...
      console.log('-----------getMinTick(TICK_SPACINGS[FeeAmount.MEDIUM])----------', getMinTick(TICK_SPACINGS[FeeAmount.MEDIUM]))
      console.log('-----------getMaxTick(TICK_SPACINGS[FeeAmount.MEDIUM])----------', getMaxTick(TICK_SPACINGS[FeeAmount.MEDIUM]))
    ...
    describe('10k of token0 fees collect', () => {
      ...
      it('actually collected', async () => {
        ...
        await expect(
          ...
          .to.emit(tokens[0], 'Transfer')
          .withArgs(poolAddress, wallet.address, 1256)
          .to.not.emit(tokens[1], 'Transfer')
      })
    })
  })
